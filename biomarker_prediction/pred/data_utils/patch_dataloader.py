...
class MSIDataset(data.Dataset):
    def __init__(self, file_path = [], MCO = True, train = True, batch_size = 64):
        ...
        if self.MCO:
            id_msi_df = pd.read_csv('/data/cm/MSI_new/MCO_MSI.csv')
            self.id_msi = id_msi_df[['ParentSpecimen','msi_status']]
            self.id_msi.columns = ['ID','MSI'] 
            # 'MSS','MSI'
        if not self.MCO:
            id_msi_df = pd.read_csv('/data/cm/MSI_new/Exp5_TEST_RESULT_PATIENT_SCORES_FULL.csv')
...
class MSIDataset_inter(data.Dataset):
    def __init__(self, file_path = [], MCO = True, train = True, batch_size = 64):
        ...
        if self.MCO:
            ...
            # 'MSS','MSI'
        if not self.MCO:
            id_msi_df = pd.read_csv('/data/cm/MSI_new/TCGA_labels.csv')
