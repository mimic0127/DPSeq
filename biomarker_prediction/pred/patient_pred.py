...
#%%
# Sequencer
class Sequencer_Net(nn.Module):
    def __init__(self):
        super(Sequencer_Net, self).__init__()
        self.backbone = sequencer2d_s()
        self.backbone.head = nn.Linear(384,256)
    ...
    def forward(self, x):
        x = self.backbone(x)  # [batch_size, 384]
        x = self.relu(x)
        x = self.dropout(x)
        x = self.fc2(x)
        x = self.relu(x)
...
def predict_patient_level(patient_path, model, device, MCO = False):
    """
    ...
    #random.seed(3455)
    #random.shuffle(patch_path_ls)
    #patch_path_ls = patch_path_ls[:500]
   ...
   #time_total = pred_time + time_total
    #MSI_pred_avg = np.median(pred_list)
    ...
    MSI_pred_new = [x for x in pred_list if x<0.2 or x >0.6]
    MSI_pred_avg_36 = np.array(MSI_pred_new).mean()
    print(MSI_pred_avg,MSI_true)
    
    return MSI_true,MSI_pred_avg, MSI_pred_top1, MSI_pred_top10, MSI_pred_top20,MSI_pred_top30, MSI_pred_top50,MSI_pred_avg_36
...
if __name__ == '__main__':
    ...
    for i in test_paths:
        ...
        if len(glob.glob(i + '/*')) == 0:
            t.append(i)
            continue
        test_path_tosave.append(i)
        msi_true, MSI_pred_avg, MSI_pred_top1, MSI_pred_top10, MSI_pred_top20,MSI_pred_top30, MSI_pred_top50, MSI_pred_avg_36 = predict_patient_level(i, model_test, device = device,MCO = False)
        ...
        pred_test_36.append(MSI_pred_avg_36)
        k = k+1
        print(k)
    print(t)
    print(k) 
    pred_df = pd.DataFrame({'test_paths':test_path_tosave,
                            ...
                            'pred_36':pred_test_36,
                            'true_test':true_test})
    pred_df.to_csv(outputdir + '/prediction_results_new/{}_pred.csv'.format(model_name))
    ...
    test_auc_list_36.append(test_auc_36)

    result = pd.DataFrame({'test_auc':test_auc_list,'test_auc_36':test_auc_list_36,'test_auc_top1':test_auc_list_top1,'test_auc_top10':test_auc_list_top10,'test_auc_top20':test_auc_list_top20,'test_auc_top30':test_auc_list_top30,'test_auc_top50':test_auc_list_top50})
    print(result)
    result.to_csv(outputdir + '/prediction_results_new/{}_auc.csv'.format(model_name))
