...
def get_args():
    ...
    parser.add_argument(
        '-tr', '--train_dir', default="/data/cm/MSI/NCT-CRC-HE-100K",
        help='Train_dir Path')
    parser.add_argument('-te', '--test_dir', default="/data/cm/MSI/CRC-VAL-HE-7K",
                        help='Test_dir Path')
...
if __name__ == ('__main__'):
    ...
    pre_train_model = DPSeq()
    model_dict =  pre_train_model.backbone.state_dict()
    saved_model = torch.load('/data/cm/NLP_based_model_code/pretrained_model/sequencer2d_s.pth')
    state_dict = {k:v for k,v in saved_model.items() if k in model_dict.keys()}
    #print(state_dict.keys())  # dict_keys(['w', 'conv1.weight', 'conv1.bias', 'conv2.weight', 'conv2.bias'])
    del state_dict['head.weight']
    del state_dict['head.bias']
    model_dict.update(state_dict)
    pre_train_model.backbone.load_state_dict(model_dict)
    ...
    for epoch in range(0, 10):
        ...
        torch.save(model.state_dict(), args.save_path + 'Seuqnecer_pretrained' + str(epoch) + '.pth')
